...
scrape_job_page <- function(url) {
  ...
  # Extract "About the project"
  about_project <- html_elements(content, 'ul[data-test="text-about-project"] li') %>%
    html_text(trim = TRUE) %>%
    paste(collapse = "; ")
  
  # Extract "Your responsibilities"
  responsibilities <- html_elements(content, 'section[data-test="section-responsibilities"] ul[data-test="aggregate-bullet-model"] li') %>%
    html_text(trim = TRUE) %>%
    paste(collapse = "; ")
  
  # Extract "Our requirements"
  requirements <- html_elements(content, 'section[data-test="section-requirements"] ul[data-test="aggregate-bullet-model"] li') %>%
    html_text(trim = TRUE) %>%
    paste(collapse = "; ")
  ...
  technologies_expected <- if(length(expected_techs) > 0) stri_join(expected_techs, collapse = ", ") else NA
  technologies_optional <- if(length(optional_techs) > 0) stri_join(optional_techs, collapse = ", ") else NA
...
main_scraper <- function() {
  base_url <- "https://www.pracuj.pl/praca?cc=5002001%2C5003001%2C5037004%2C5037005%2C5008007%2C5009009%2C5011005%2C5011003%2C5015001%2C5016001%2C5016005%2C5034001%2C5018009%2C5018008%2C5018007%2C5018006%2C5024004%2C5032001"
...
jobs_df_scrapped[jobs_df_scrapped == ""] <- NA
